...
public class MapParser {
  ...
  private void makeGrid(char[][] map, int width, int height, Square[][] grid,
                        List<Ghost> ghosts, List<Square> startPositions) {
  ...
  protected void addSquare(Square[][] grid, List<Ghost> ghosts,
    ...
    case 'P':
      ...
      break;
    default:
      throw new PacmanConfigurationException("Invalid character at " + x + "," +
                                             y + ": " + c);
    }
  }
  ...
  public Level
  parseMap(String mapName) throws IOException {
    try (InputStream boardStream =
             MapParser.class.getResourceAsStream(mapName)) {
      if (boardStream == null) {
        throw new PacmanConfigurationException("Could not get resource for: " +
                                               mapName);
      }
   ...
   */
  protected BoardFactory getBoardCreator() { return boardCreator; }
}
